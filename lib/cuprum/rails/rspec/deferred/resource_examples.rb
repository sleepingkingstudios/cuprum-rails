...
module Cuprum::Rails::RSpec::Deferred
  # Deferred examples for validating resource implementations.
  module ResourceExamples
    include RSpec::SleepingKingStudios::Deferred::Provider
    include Cuprum::Collections::RSpec::Deferred::ResourceExamples

    deferred_examples 'should be a Rails Resource' do |book_class = 'Book'|
      deferred_context 'with a plural parent resource' do
        let(:authors_resource)    { described_class.new(name: 'authors') }
        let(:constructor_options) { super().merge(parent: authors_resource) }
      end

      deferred_context 'with a singular parent resource' do
      ...
      deferred_context 'with multiple ancestor resources' do
      ...
      example_class 'Grimoire',         book_class
      example_class 'Spec::ScopedBook', book_class
      ...
      describe '#actions' do
        ...
        context 'when initialized with singular: true' do
          ...
          let(:expected) do
            Set.new(described_class::SINGULAR_ACTIONS)
          end
      ...
      describe '#ancestors' do
        ...
        wrap_deferred 'with a plural parent resource' do
          let(:expected) { [authors_resource, resource] }

          it { expect(resource.ancestors).to be == expected }
        end

        wrap_deferred 'with a singular parent resource' do
          let(:expected) { [library_resource, resource] }

          it { expect(resource.ancestors).to be == expected }
        end

        wrap_deferred 'with multiple ancestor resources' do
      ...
      describe '#permitted_attributes' do
        ...
        context 'when initialized with permitted attributes' do
          ...
          it 'should return the permitted attributes' do
            expect(resource.permitted_attributes).to be == permitted_attributes
          end
        end
      end
      ...
      describe '#routes' do
        ...
        it 'should return a plural routes object' do
          expect(resource.routes).to be_a Cuprum::Rails::Routing::PluralRoutes
        end
