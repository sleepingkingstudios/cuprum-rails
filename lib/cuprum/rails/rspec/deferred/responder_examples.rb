...
module Cuprum::Rails::RSpec::Deferred
  ...
  module ResponderExamples
    ...
    #
    # - #result: The result passed to the responder.
    #
    # The behavior can be customized by defining the following methods:
    #
    # - #response: The response being tested. Defaults to the value of calling
    ...
    do |fallback_location: '/', flash: {}, status: 302|
      ...
      depends_on :result, 'the result passed to the responder'
      ...
      it { expect(response.fallback_location).to match(expected_fallback) }
    ...
    deferred_examples 'should redirect to' do |path, flash: {}, status: 302|
      ...
      it { expect(response.path).to match(expected_path) }
    ...
    do |template, assigns: {}, flash: {}, layout: nil, status: 200|
      ...
      it { expect(response.template).to match(expected_template) }

      it { expect(response.assigns).to match(expected_assigns) }

      it { expect(response.layout).to match(expected_layout) }
